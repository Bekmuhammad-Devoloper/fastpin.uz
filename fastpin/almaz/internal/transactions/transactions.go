...
func NewTranactionHandler(router *http.ServeMux, deps *TransactionhandlerDeps) *TransactionHandler {
	...
	router.Handle("/transactions/transactionsCreate", deps.AdminMW(http.HandlerFunc(handler.create())))
	router.Handle("/transactions/transactionsDelete", deps.AdminMW(http.HandlerFunc(handler.delete())))
	router.Handle("/transactions/getTransactionsByPeriod", deps.AdminMW(http.HandlerFunc(handler.getTransactionsByPeriod())))
	router.HandleFunc("/transactions/transactionsGet", handler.getTransactions())
	return handler
}
...
func (handler *TransactionHandler) getTransactions() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		body, err := req.HandleBody[getTransactionsRequest](&w, r)
		if err != nil {
			res.Json(w, err, 400)
			return
		}
...
func (handler *TransactionHandler) getTransactionsByPeriod() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err = handler.TransactionRepository.DataBase.
			Where("CONCAT(year::text, LPAD(month::text, 2, '0'), LPAD(day::text, 2, '0')) >= ?", startYMD).
