...
func main() {
	...
	authMW := middleware.Auth(conf.JWTSecret, authRepo)
	adminMW := func(h http.Handler) http.Handler { return authMW(middleware.AdminOnly(h)) }
	...
	database.AutoMigrate(&buy.Refund{})
