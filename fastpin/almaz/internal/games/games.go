...
func NewGamesHandler(router *http.ServeMux, deps GameshandlerDeps) *GamesHandler {
	...
	router.Handle("/games/create", deps.AdminMW(http.HandlerFunc(handler.create())))
	router.Handle("/games/getGameById", deps.AuthMW(http.HandlerFunc(handler.getGameById())))
	router.Handle("/games/updateGame", deps.AdminMW(http.HandlerFunc(handler.updateGame())))
	router.Handle("/games/deleteGame", deps.AdminMW(http.HandlerFunc(handler.deleteGame())))
	return handler
}
